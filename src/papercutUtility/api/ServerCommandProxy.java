package papercutUtility.api;

/*
 * (c) Copyright 1999-2012 PaperCut Software International Pty. Ltd.
 * $Id: ServerCommandProxy.java 23150 2013-07-18 23:32:15Z geoff $
 */
...
public class ServerCommandProxy {
     ...
     *
     * @param userName The name of the user.
     * @param propertyName The name of the property to set.  Valid options include: balance, card-number, card-number2,
     * card-pin, department, disabled-net, disabled-print, email, full-name, notes, office,
     * password print-stats.job-count, print-stats.page-count, net-stats.data-mb, net-stats.time-hours, restricted.
    ...
    public void setUserAccountSelectionStandardPopup(String username, boolean allowPersonal,
        ...
        setUserAccountSelectionStandardPopup(username, allowPersonal, allowListSelection,
                allowPinCode, allowPrintingAsOtherUser, chargeToPersonalWhenSharedSelected, null);
    }
    ...
    */
    ...
    public boolean createUserClientAccountsFile() {
        Vector<Object> params = new Vector<Object>();
        params.add(_authToken);
        Boolean r = (Boolean) execute("api.createUserClientAccountsFile", params);
        return r.booleanValue();
    }

    /**
     * Return true if the server successfully been configured... i.e. the inital configuration wizard
     * has been completed.  This method is currently used by the Mac register printer code/script.
     * 
     * NOTE:  This is only used internally so no need to make public in the public version of
     *           ServerCommandProxy.
     * 
     * @return A boolean indicating indicating TRUE if the server is complete, else FALSE.
     */
    public boolean isServerSetupComplete() {
        Vector<Object> params = new Vector<Object>();
        params.add(_authToken);
        Boolean r = (Boolean) execute("api.isServerSetupComplete", params);
        return r.booleanValue();
    }
